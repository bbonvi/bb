...
pub fn fetch_via_microlink(url: &str) -> Result<Option<Metadata>> {
    ...
    let mic_result = crate::microlink::microlink(url, api_key.as_deref());
...
pub fn fetch_meta(url: &str, opts: MetaOptions) -> Result<Metadata> {
    ...
    if let Some(mut meta) = fetch_via_microlink(url)? {
        if meta.icon_url.is_none() {
