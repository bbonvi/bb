...
pub fn fetch_meta(url: &str, opts: MetaOptions) -> Result<Metadata> {
    ...
    if let Some(mut metadata) = registry.fetch_metadata(url, &opts)? {
        ...
        if metadata.image.is_none() {
            metadata.try_fetch_image();
        }
        if metadata.icon.is_none() {
            metadata.try_fetch_icon();
        }
    ...
    if metadata.icon.is_none() {
        try_fetch_ddg_favicon(&mut metadata, url);
    }
...
fn fallback_fetch(url: &str, _opts: &MetaOptions) -> Result<Metadata> {
    // Try DuckDuckGo fallback
    if let Some(mut meta) = crate::scrape::get_data_from_ddg(url) {
        if meta.icon.is_none() {
            if let Some(ref icon_url) = meta.icon_url {
                if let Some(bytes) = fetchers::fetch_bytes(icon_url) {
